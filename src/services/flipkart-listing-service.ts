...
import { FlipkartListing, CreateFlipkartListingInput, ListingStatus } from '../types/listing'
import { WebhookService } from '../webhooks/webhook-service'

export class FlipkartListingService {
  constructor(
    private readonly listings: FlipkartListingRepository,
  ...
  async create(input: CreateFlipkartListingInput) {
    const now = new Date().toISOString()
    const listing: FlipkartListing = {
      id: randomUUID(),
      platform: 'flipkart',
  ...
  private async transition(listing: FlipkartListing, nextStatus: ListingStatus) {
